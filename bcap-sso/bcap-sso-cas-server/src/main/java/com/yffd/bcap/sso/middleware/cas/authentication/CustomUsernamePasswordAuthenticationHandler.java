...
public class CustomUsernamePasswordAuthenticationHandler extends AbstractUsernamePasswordAuthenticationHandler {
    ...
    protected HandlerResult authenticateUsernamePasswordInternal(UsernamePasswordCredential transformedCredential,
        ...
        LOG.info("自定义认证处理器>>CustomUsernamePasswordAuthenticationHandler，username：{}, password：{}, originalPassword：{}", transformedCredential.getUsername(), transformedCredential.getPassword(), originalPassword);
        if("guest".equals(transformedCredential.getUsername())){
