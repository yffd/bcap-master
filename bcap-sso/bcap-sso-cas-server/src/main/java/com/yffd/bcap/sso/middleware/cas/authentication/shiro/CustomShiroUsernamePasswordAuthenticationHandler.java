...
/**
 * cas服务端自定义集成shiro
 * 参考：{@link org.apereo.cas.adaptors.generic.ShiroAuthenticationHandler}
 */
public class CustomShiroUsernamePasswordAuthenticationHandler extends AbstractUsernamePasswordAuthenticationHandler {
    ...
    protected HandlerResult authenticateUsernamePasswordInternal(UsernamePasswordCredential transformedCredential, String originalPassword) throws GeneralSecurityException, PreventedException {
        LOG.info("自定义认证处理器>>CustomShiroUsernamePasswordAuthenticationHandler，username：{}, password：{}, originalPassword：{}", transformedCredential.getUsername(), transformedCredential.getPassword(), originalPassword);
        try {
    ...
    protected void checkSubjectRolesAndPermissions(final Subject currentUser) throws FailedLoginException {
        LOG.info("自定义认证处理器>>CustomShiroUsernamePasswordAuthenticationHandler#checkSubjectRolesAndPermissions");
        //TODO
